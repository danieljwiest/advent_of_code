...
function isValidMarker () {
   if (startOfPacketMarker.length === PACKET_MARKER_LENGTH && !hasDuplicateCharacters(inputHash)) {
    return true;
   } else {
    return false;
   }
...
function moveToNextChar () {
    ...
    if (startOfPacketMarker.length === PACKET_MARKER_LENGTH) {
        inputHash[firstChar]--;
